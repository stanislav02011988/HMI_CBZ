...
class SettingsProject(QObject):
    ...
    @Slot("QVariant")
    def save_block_installation_settings(self, dict) -> None:
        items = self._json_menager.read_json_file(self._file_path, self._file_name) or {}
        items["block_installation_settings"] = {
            "name_installation" : dict["name_installation"],
            "type_installation" : dict["type_installation"],
            "inf_number" : dict["inf_number"],
            "installation_number" : dict["installation_number"],
            "year_installation" : dict["year_installation"]
        }
        self._json_menager.write_json_file(path_folder=self._file_path, file_name=self._file_name, items=items)
        self._add_watch()
        self.signalLoadFile.emit()
