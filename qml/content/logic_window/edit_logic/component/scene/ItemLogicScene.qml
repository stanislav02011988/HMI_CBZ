...
import qml.managers
import "./dialog_resize_element_logic_map"
...
Item {
    ...
    MouseArea {
        ...
        onClicked: (mouse) => {
            if (mouse.button === Qt.RightButton && QmlLogicMapScene.selectedItems.length < 2 && wrapper.editMode) {
                contextMenu.popup()
                mouse.accepted = true
                return
            }
        ...
        onPositionChanged: (mouse) => {
            ...
            const itemsToMove = QmlLogicMapScene.selectedItems.length > 1 ? QmlLogicMapScene.selectedItems.slice() : [wrapper]
    ...
    Keys.onPressed: (event) => {
            ...
            const itemsToMove = QmlLogicMapScene.selectedItems.length > 1
                              ? QmlLogicMapScene.selectedItems.slice()
                              : [wrapper]
